var Dest = function () {
    ...
    var addUpdate = function (id) {
        $.ajax({
            type: 'Get',
            url: '/Admin/Destination/AddUpdate',
            dataType: 'html',
            data: { id: id },
            success: function (content, strStatus) {
                ...
                $('#saveDestinationfrm').submit(function (e) {
                    ...
                    // Append files from Dropzone to the form data
                    myDropzone.getAcceptedFiles().forEach(function (file, index) {
                        // Append the file to formData with a unique key
                        /*formData.append('Images\[' + index +'\]', file);*/ // e.g., 'file0', 'file1', etc.
                        //formData.append('Images', file); // e.g., 'file0', 'file1', etc.
                    ...
                    $.ajax({
                        url: '/Admin/Destination/SaveDestination', // Change this to your server-side script
                        type: 'POST',
                        data: formData,
    ...
    this.init = function () {
        ...
        $('#btn-search').on('click', function () {
            window.location.href = "/Admin/Destination/Index?keyword=" + $('#Keyword').val();
        })
        ...
        $('.btn-delete').on('click', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '/Admin/Destination/SwitchDelete',
                dataType: 'JSON',
                data: { id: Id },
                success: function (content, strStatus) {
                    showSuccessMessage(content.message, function () {
